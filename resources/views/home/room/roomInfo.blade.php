        ...
        <div class="bgo-content ">
            <div class="col-xs-8 mt55 order-content ">
                <p style="font-size: 24px; padding-top: 20px;">您已选择：
                    <span style="color: #aa0017;">仙女山华邦酒店-{{ $room->title }}</span>
                    <span style="color: #aa0017; padding-left: 24px;">¥{{ $room->price }}元/间</span></p>
                    ...
                    <form method="post" action="{{ url('room/orderRoom') }}">
                        ...
                        <div class="form-group order-date">
				     				<span class="input-group">
										<label>入住</label>
										<input class="laydate-icon get_time" value="{{ old('get_time') }}" placeholder="{{ $errors->first('get_time')  }}" name="get_time" onclick="laydate({istime: true, format: 'YYYY-MM-DD'})">
									</span>
									<span class="input-group">
										<label>退房</label>
										<input class="laydate-icon out_time" name="out_time" placeholder="{{ $errors->first('out_time')  }}" onclick="laydate({istime: true, format: 'YYYY-MM-DD'})">
									</span>
                        </div>
