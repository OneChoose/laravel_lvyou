    ...
    <div class="row">
        ...
        <div class="col-xs-10">
            ...
            <div class="container-fluid" id="content_box_blank">
                <form id="form_add" class="form-horizontal" method="post" action="/admin/goods_type/update" enctype="multipart/form-data">
                    <div class="row">
                        <div class="col-xs-12">
                            {{ csrf_field() }}
                            <input type="hidden" value="{{ $goods_type->id }}" name="id"/>
                            <div class="row form-group">
                                <label class="col-xs-2 control-label">商品分类名称</label>
                                <div class="col-xs-6">
                                    <input type="text" class="form-control" name="name" value="{{ $goods_type->name }}" placeholder="{{ $errors->first('name')  }}" data-rule="required;">
                                </div>
                            </div>
                            <div class="row form-group">
                                ...
                                <div class="col-xs-6">
                                    <select class="form-control c" name="parent" id="driver_area">
                                        ...
                                        @foreach($goods_types as $v)
                                            @if($v->parent_id == 0)
                                                <option value="{{ $v->id }}"  {{ ($goods_type->parent_id == $v->id ) ? "selected='selectet'": '' }}>{{ $v->name }}</option>
                                                @foreach($v->childrenGoodsType as $list)
                                                    <option disabled="disabled" value="{{ $list->id }}"  {{ ($goods_type->parent_id == $list->id ) ? "selected='selectet'": '' }}>|--{{ $list->name }}</option>
                                                @endforeach
                                            @endif
                                        @endforeach
                                    </select>
                            ...
                            <div class="row form-group">
                                ...
                                <label class="col-xs-0 control-label"><img width="160px" src="{{ $goods_type->picurl }}"/></label>
                            </div>
                            <div class="row form-group">
                                <label class="col-xs-2 control-label">跳转链接</label>
                                <div class="col-xs-6">
                                    <input type="text" class="form-control" value="{{ $goods_type->linkurl }}" name="linkurl" data-rule="required;">
                                </div>
                            </div>
                            <div class="row form-group">
                                <label class="col-xs-2 control-label">排序</label>
                                <div class="col-xs-6">
                                    <input type="text" class="form-control" value="{{ $goods_type->sort }}" name="sort" data-rule="required;">
                                </div>
                            </div>
                            <div class="row form-group">
                                <label class="col-xs-2 control-label">隐藏类别</label>
                                <div class="col-xs-6">
                                    <td><input type="radio" <?php echo ($goods_type->checkinfo == 1) ? 'checked="checked"': '';?>  name="checkinfo" value="true">
                                        显示&nbsp;
                                        <input type="radio" <?php echo ($goods_type->checkinfo == 0) ? 'checked="checked"': '';?>  name="checkinfo" value="false">
                                        隐藏</td>
                                </div>
                            </div>
                        </div>
